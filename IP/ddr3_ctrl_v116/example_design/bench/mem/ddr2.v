...
`endif
    ...
    begin
        ...
        casex ({samebank, fromcmd, cmd})
            ...
            {1'b1, ACTIVATE , PRECHARGE} : begin if (($time - tm_bank_activate[bank] > TRAS_MAX) && (active_bank[bank] === 1'b1) && !disable_trfc_max)                    $display ("%m: at time %t ERROR:  tRAS maximum violation during %s to bank %d", $time, cmd_string[cmd], bank);
...
//                              $display("%m: at time %t WARNING: NOP or DESELECT is required for 200 us before CKE is brought high", $time);
    ...
    task data_task;
        ...
        begin
            ...
            if (diff_ck) begin
                for (i=0; i<36; i=i+1) begin
                    if (dq_in_valid && dll_locked && ($time - tm_dqs_neg[i] < $rtoi(TDSS*tck_avg))) begin
                        $display ("%m: at time %t ERROR: tDSS violation on %s bit %d", $time, dqs_string[i/18], i%18);
                        $display("i = %1d, tm_dqs_neg[%1d] = %1d", i, i, tm_dqs_neg[i]);
                    end
                ...
                if (dll_locked) begin
                    ...
                    for (i=0; i<50; i=i+1) begin
                        ...
                        case (i)
                          ...
                          5,6,7,8,9 : if ((!disable_terr) && (terr_nper_rtime - TERR_N1PER >= 1.0)) $display ("%m: at time %t ERROR: tERR(n1per) violation by %f ps.", $time, terr_nper_rtime - TERR_N1PER);
                            default : if ((!disable_terr) && (terr_nper_rtime - TERR_N2PER >= 1.0)) $display ("%m: at time %t ERROR: tERR(n2per) violation by %f ps.", $time, terr_nper_rtime - TERR_N2PER);
                        endcase
                    end
    ...
    begin
        if (dqs_in_valid && ((wdqs_pos_cntr[i] < burst_length/2) || b2b_write) && (dqs_n_en || i<18)) begin
            if (dqs_in[i] ^ prev_dqs_in[i]) begin
                ...
                if ($time - tm_dm[i%18] < TDS && !disable_tdstdh) 
    ...
    begin
        ...
        //marked by jiangde tm_dqs_neg[i] = $time;
        prev_dqs_in[i] <= dqs_in[i];
        //added by jiangde: to advoid the race condition when checking tDSS
        #1;
        tm_dqs_neg[i] = (dqs_in[i] === 1'b0) ? $time - 1 : tm_dqs_neg[i];
    end
    endtask
