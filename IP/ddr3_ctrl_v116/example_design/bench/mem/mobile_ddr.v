...
`ifdef den128Mb
    `include "../../example_design/bench/mem/128Mb_mobile_ddr_parameters.vh"
`elsif den256Mb
    `include "../../example_design/bench/mem/256Mb_mobile_ddr_parameters.vh"
`elsif den512Mb
    `include "../../example_design/bench/mem/512Mb_mobile_ddr_parameters.vh"
`elsif den1024Mb
    `include "../../example_design/bench/mem/1024Mb_mobile_ddr_parameters.vh"
`elsif den2048Mb
    `include "../../example_design/bench/mem/2048Mb_mobile_ddr_parameters.vh"
`else
...
`endif
    ...
    //differential clk
    reg            diff_ck;
    // Initialize diff_ck to 1 to avoid setup violations at startup
    wire diff_ck_wire = diff_ck;
    initial diff_ck = 1'b1;
...
//        MRD_chk = 0;
    ...
    always @(diff_ck) begin
        ...
        if (Cke & Clk_Chk_enable) begin
            ...
            if ((pos_clk_time / clk_period < tCH_MIN) && ~clock_stop && ~prev_clock_stop && ($realtime > 2 * clk_period)) begin
            ...
            if ((neg_clk_time / clk_period < tCL_MIN) && ~clock_stop && ~prev_clock_stop && ($realtime > 2 * clk_period)) begin
...
//                end
    ...
    begin
        ...
        tWR_cycle = ceil((tWR - 0.001) / clk_period); // Using ceil for round up to next integer
...
//        Burst_Terminate_Pipeline;
    ...
    always @ (posedge Dqs_in[0]) begin
        ...
        if(Dqs_in[0])begin    //added by quyi to make sure the posedge is a real posedge and avoid the simulator not work normally
    ...
    always @ (posedge Dqs_in[1]) begin
        // Latch data at posedge Dqs
        if(Dqs_in[1])begin    //added by quyi
...
`ifdef x32
    always @ (posedge Dqs_in[2]) begin
        // Latch data at posedge Dqs
        if(Dqs_in[2])begin    //added by quyi
    ...
    always @ (posedge Dqs_in[3]) begin
        // Latch data at posedge Dqs
        if(Dqs_in[3])begin    //added by quyi
...
`endif
    ...
    always @ (negedge Dqs_in[0]) begin
        ...
        if(!Dqs_in[0])begin    //added by quyi
...
`else `define sg10                                 //              specparams for -10 (CL = 3)
        ...
        specparam tDQSL            =     3.84;     // tDQSL  ns    DQS input Low Pulse Width
`endif
        // CK pulse width should not be checked when CKE is low
        $period   (posedge Clk &&& (Cke === 1), tCLK_MIN);
        ...
        $setuphold(posedge diff_ck_wire,   Cke,   tIS, tIH);
        //$setuphold(posedge diff_ck_wire,   Cs_n,  tIS, tIH);    //modified by jiangde on 2017.05.02
        //$setuphold(posedge diff_ck_wire,   Cas_n, tIS, tIH);    //modified by jiangde on 2017.05.02
        //$setuphold(posedge diff_ck_wire,   Ras_n, tIS, tIH);    //modified by jiangde on 2017.05.02
        //$setuphold(posedge diff_ck_wire,   We_n,  tIS, tIH);    //modified by jiangde on 2017.05.02
        $setuphold(posedge diff_ck_wire &&& (Cke === 1),   Cs_n,  tIS, tIH);
        $setuphold(posedge diff_ck_wire &&& (Cke === 1),   Cas_n, tIS, tIH);
        $setuphold(posedge diff_ck_wire &&& (Cke === 1),   Ras_n, tIS, tIH);
        $setuphold(posedge diff_ck_wire &&& (Cke === 1),   We_n,  tIS, tIH);
        $setuphold(posedge diff_ck_wire,   Addr,  tIS, tIH);
        $setuphold(posedge diff_ck_wire,   Ba,    tIS, tIH);
//        $setuphold(posedge diff_ck_wire, negedge Dqs &&& wdqs_valid, tDSS , tDSH);
    endspecify
